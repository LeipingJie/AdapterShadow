...
class SAMNetFramework(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.args.freeze_backbone:
            for p in self.model.mask_generator.efficient_encoder.parameters():
    ...
    def _calculate_train_loss(self, batch, batch_idx):
        ...
        if self.type=='gen_mask':
            # mask from CNN
            pred_mask = self.model.mask_generator(imgs, imgs.shape[-2:])
            ...
            if self.args.vpt:
                pred_mask, vpt_prompts = self.model.mask_generator(imgs[:, :, ::4, ::4] if self.args.small_size else imgs, imgs.shape[-2:], self.args.vpt)
            else:
                pred_mask = self.model.mask_generator(imgs[:, :, ::4, ::4], imgs.shape[-2:])

            m_b, m_c, m_h, m_w = pred_mask.shape
            
            # topk points
            if self.args.sample=='topk':
                ...
                pt = (coords_torch, labels_torch) #  [2, 10, 2]   [2, 10]
                se, de = self.model.prompt_encoder(points=pt, boxes=None, masks=None, )
                pred_mask = pred_mask.view(m_b, m_c, m_h, m_w)
            # grid sampling
            elif self.args.sample=='grid':
                amp2d = nn.AdaptiveMaxPool2d((self.args.grid_out_size, self.args.grid_out_size), return_indices=True)
                values, indices = amp2d(pred_mask)
                values = values.flatten(1)
                indices = indices.flatten(2)
                hs = torch.div(indices, m_w, rounding_mode='floor')
                ws = indices % m_w
                coords_torch = torch.cat((hs, ws), 1).permute(0, 2, 1) # [2, 1, 16] [2, 1, 16]
                labels_torch = torch.gt(values, self.args.grid_thres).float()
                
                pt = (coords_torch, labels_torch) # [2, 16, 2] [2, 16]
                se, de = self.model.prompt_encoder(points=pt, boxes=None, masks=None, )
        ...
        imge = self.model.image_encoder(imgs, vpt_prompts if self.args.vpt else None)
